...
class BasicCombatRules:
    ...
    def at_defeat(self, defeated):
        ...
        Notes:
            ...
            do it.
        """
        defeated.location.msg_contents("%s has been defeated!" % defeated.get_display_name())
    ...
    def resolve_attack(self, attacker, defender, attack_value=None, defense_value=None):
        ...
        if attack_value < defense_value:
            attacker.location.msg_contents("%s's attack misses %s!" % (attacker.get_display_name(), defender.get_display_name()))
        else:
            damage_value = self.get_damage(attacker, defender)  # Calculate damage value.
            # Announce damage dealt and apply damage.
            attacker.location.msg_contents(
                "%s hits %s for %i damage!" % (attacker.get_display_name(), defender.get_display_name(), damage_value)
            )
...
class TurnHandler(DefaultScript):
    ...
    def next_turn(self):
        ...
        if defeated_characters == (
            ...
            for fighter in self.db.fighters:
                if fighter.db.HP != 0:
                    LastStanding = fighter.get_display_name()  # Pick the one fighter left with HP remaining
        ...
        self.obj.msg_contents("%s's turn ends - %s's turn begins!" % (currentchar.get_display_name(), newchar.get_display_name()))
