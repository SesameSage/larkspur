...
class CycleWeather(Script):
    ...
    def at_repeat(self, **kwargs):
        if len(self.db.weathers) < 1:
            return
        current_weather = self.db.current_weather
        new_weather = random.choices(population=self.db.weathers, weights=self.db.weathers.values())[0]
        if current_weather != new_weather:
            self.obj.update_weather(new_weather)
